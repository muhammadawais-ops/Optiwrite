...
- Competitor Edge: `;
  ...
  const getSemanticVariations = async () => {
    ...
    try {
      ...
      const variations = await withRetry(async () => {
        const response = await ai.models.generateContent({
          model: GEMINI_MODEL,
          contents: prompt,
          config: { responseMimeType: "application/json" }
    ...
    try {
      ...
      const streamFn = async () => {
        ...
        for await (const chunk of responseStream) {
          ...
          // Set partial content for live preview
          // We try to extract "content": "..." from the growing JSON string
          const contentMatch = fullStreamedText.match(/"content"\s*:\s*"(.*)"\s*}/s) || fullStreamedText.match(/"content"\s*:\s*"(.*)/s);
          if (contentMatch && contentMatch[1]) {
            // Unescape the JSON string part roughly for preview
            const partial = contentMatch[1].replace(/\\n/g, '\n').replace(/\\"/g, '"');
            setState(s => ({ ...s, generatedContent: partial }));
          }
        }
        return fullStreamedText;
      };

      const finalResultText = await withRetry(streamFn);
      const data = safeJsonParse<{ metaTitle: string; metaDescription: string; content: string }>(finalResultText, {
        metaTitle: '',
        metaDescription: '',
        content: ''
      });

      if (!data.content) throw new Error("Empty content in response");
  ...
  const generateFaqs = async () => {
    ...
    try {
      ...
      const faqs = await withRetry(async () => {
        const response = await ai.models.generateContent({
          model: GEMINI_MODEL,
          contents: prompt,
        });
